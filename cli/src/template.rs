...
pub fn generate_template(output_path: &Path, with_glyphs: bool) -> Result<(), String> {
    ...
    log!("テンプレート生成: {w}x{h} px (A4 300dpi)");
    ...
    log!("テンプレート保存: {}", output_path.display());
...
fn draw_markers(img: &mut RgbaImage) {
    ...
    log!("  マーカー描画完了 (TL=filled, TR/BL/BR=outline)");
}
...
fn draw_center_marker(img: &mut RgbaImage) {
    ...
    log!("  中心マーカー描画完了 ({}x{}mm 塗りつぶし四角、白境界付き)", layout::CENTER_MARKER_SIZE, layout::CENTER_MARKER_SIZE);
}
...
fn draw_cell_grid(img: &mut RgbaImage) {
    ...
    log!("  セルグリッド描画完了 ({}列×{}行×2マス)", layout::COLS, layout::ROWS);
}
...
fn draw_gray_bars(img: &mut RgbaImage) {
    ...
    log!("  グレースケールバー描画完了 ({}段階)", layout::GRAY_BAR_STEPS);
}
...
fn draw_dummy_glyphs(img: &mut RgbaImage) {
    ...
    // (row, col, cell_idx, fill_ratio, has_check)
    // has_check: false=空欄, true=✓
    let test_cells: &[(usize, usize, usize, f64, bool)] = &[
    ...
    for &(row, col, cell_idx, fill_ratio, has_check) in test_cells {
        // 文字領域にダミー描画
        let (mm_x, mm_y) = layout::get_cell_position(row, col, cell_idx);
        let inner_px = layout::mm_to_px(layout::INNER_SIZE).round() as u32;
    ...
    log!("  ダミー文字+チェックマーク描画完了 (採用判定テスト用)");
}
