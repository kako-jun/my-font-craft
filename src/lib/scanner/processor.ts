...
export function translateWasmError(rawError: string, buildSha?: string | null): string {
  ...
  if (rawError.includes('マーカーが検出できませんでした')) {
    ...
    rawError.includes('マーカーの配置が不正') ||
    rawError.includes('四隅マーカーらしい形状が見つかりません')
  ) {
    // 四隅マーカー誤検出（#115）: マーカーが白塗り/隠れ等で欠落し、別の印刷内容を
    // マーカーと誤検出したケース（ブロブ形状検証 or クアッド幾何検証で棄却）。
    // QR不鮮明ではなくマーカー段階のガイドを出す。
    msg =
      '四隅のマーカーを正しく検出できませんでした。マーカーが隠れたり塗りつぶされたりせず、紙全体が写るよう、なるべく正面から撮影してください。';
  } else {
